...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			cobble_create_observer( 'cobble-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				cobble_gutenberg_editor_init();
			} );
		} else {
		 	cobble_gutenberg_editor_init();
		}
	...
	function cobble_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'cobble_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'cobble_inited' ) ) {
	...
	// Init on page load
	function cobble_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = cobble_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + COBBLE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + COBBLE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in COBBLE_STORAGE['override_classes'] ) {
				$obj.addClass( COBBLE_STORAGE['override_classes'][i].replace( '%s', COBBLE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( cobble_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && COBBLE_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'cobble_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			cobble_remove_observer( 'cobble-check-editor-styles-wrapper' );
			cobble_create_observer( 'cobble-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'cobble_inited' );
	}
} );
