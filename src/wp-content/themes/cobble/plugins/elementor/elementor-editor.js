...
jQuery( document ).ready(
	function() {
		...
		cobble_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="cobble_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.cobble_post_editor' );
						if ( link.length > 0 ) {
							cobble_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.cobble_post_editor', function(e) {
						cobble_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('cobble_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function cobble_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
