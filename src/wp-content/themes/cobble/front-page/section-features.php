<div class="front_page_section front_page_section_features<?php
	$cobble_scheme = cobble_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $cobble_scheme ) && ! cobble_is_inherit( $cobble_scheme ) ) {
		echo ' scheme_' . esc_attr( $cobble_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( cobble_get_theme_option( 'front_page_features_paddings' ) );
	if ( cobble_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$cobble_css      = '';
		$cobble_bg_image = cobble_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$cobble_anchor_icon = cobble_get_theme_option( 'front_page_features_anchor_icon' );
	$cobble_anchor_text = cobble_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( cobble_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$cobble_css      = '';
			$cobble_bg_mask  = cobble_get_theme_option( 'front_page_features_bg_mask' );
			$cobble_bg_color_type = cobble_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $cobble_bg_color_type ) {
				$cobble_bg_color = cobble_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$cobble_caption = cobble_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $cobble_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $cobble_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $cobble_caption, 'cobble_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$cobble_description = cobble_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $cobble_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $cobble_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $cobble_description ), 'cobble_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! cobble_exists_trx_addons() ) {
						...
						cobble_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
