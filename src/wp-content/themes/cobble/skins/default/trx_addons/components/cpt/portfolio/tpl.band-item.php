...
if ( ! cobble_storage_get( 'lazy_load_off' ) ) {
    cobble_storage_set( 'lazy_load_off', true );
		...
		data-details="<?php
			echo esc_attr( '<div class="post_details">'
								...
								. '<div class="post_description">'
									. $details
									. ( empty( $cobble_template_args['hide_excerpt'] )
										? '<div class="post_description_content">' . get_the_excerpt() . '</div>'
										: ''
										)
									. '<a href="' . esc_url( $link ) . '" class="theme_button post_readmore"><span class="post_readmore_label">' . esc_html__( 'Learn more', 'cobble' ) . '</span></a>'
								. '</div>'
							. '</div>'
						);
		?>"
...
>
	...
	trx_addons_get_template_part(
		...
		apply_filters(
			...
			array(
				...
				'thumb_size'    => cobble_get_thumb_size(
                                    ($args['columns'] > 1
    ...
    <div class="post_content_wrap">
        ...
        <?php if(empty( $cobble_template_args['hide_excerpt'])) { ?>
