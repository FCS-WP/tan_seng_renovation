...
if ( ! function_exists( 'cobble_storage_get' ) ) {
	/**
	 ...
	 */
	function cobble_storage_get( $var_name, $default = '' ) {
		global $COBBLE_STORAGE;
		return isset( $COBBLE_STORAGE[ $var_name ] ) ? $COBBLE_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'cobble_storage_set' ) ) {
	/**
	 ...
	 */
	function cobble_storage_set( $var_name, $value ) {
		global $COBBLE_STORAGE;
		$COBBLE_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'cobble_storage_empty' ) ) {
	/**
	 ...
	 */
	function cobble_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $COBBLE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $COBBLE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $COBBLE_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $COBBLE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'cobble_storage_isset' ) ) {
	/**
	 ...
	 */
	function cobble_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $COBBLE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $COBBLE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $COBBLE_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $COBBLE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'cobble_storage_unset' ) ) {
	/**
	 ...
	 */
	function cobble_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $COBBLE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $COBBLE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $COBBLE_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $COBBLE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'cobble_storage_inc' ) ) {
	/**
	 ...
	 */
	function cobble_storage_inc( $var_name, $value = 1 ) {
		global $COBBLE_STORAGE;
		if ( empty( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = 0;
		}
		$COBBLE_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'cobble_storage_concat' ) ) {
	/**
	 ...
	 */
	function cobble_storage_concat( $var_name, $value ) {
		global $COBBLE_STORAGE;
		if ( empty( $COBBLE_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'cobble_storage_get_array' ) ) {
	/**
	 ...
	 */
	function cobble_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $COBBLE_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $COBBLE_STORAGE[ $var_name ][ $key ] ) ? $COBBLE_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $COBBLE_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $COBBLE_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'cobble_storage_set_array' ) ) {
	/**
	 ...
	 */
	function cobble_storage_set_array( $var_name, $key, $value ) {
		global $COBBLE_STORAGE;
		if ( ! isset( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$COBBLE_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'cobble_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function cobble_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $COBBLE_STORAGE;
		if ( ! isset( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $COBBLE_STORAGE[ $var_name ][ $key ] ) ) {
			$COBBLE_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$COBBLE_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$COBBLE_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'cobble_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function cobble_storage_merge_array( $var_name, $key, $value ) {
		global $COBBLE_STORAGE;
		if ( ! isset( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$COBBLE_STORAGE[ $var_name ] = array_merge( $COBBLE_STORAGE[ $var_name ], $value );
		} else {
			$COBBLE_STORAGE[ $var_name ][ $key ] = array_merge( $COBBLE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'cobble_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function cobble_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $COBBLE_STORAGE;
		if ( ! isset( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			cobble_array_insert_after( $COBBLE_STORAGE[ $var_name ], $after, $key );
		} else {
			cobble_array_insert_after( $COBBLE_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'cobble_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function cobble_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $COBBLE_STORAGE;
		if ( ! isset( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			cobble_array_insert_before( $COBBLE_STORAGE[ $var_name ], $before, $key );
		} else {
			cobble_array_insert_before( $COBBLE_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'cobble_storage_push_array' ) ) {
	/**
	 ...
	 */
	function cobble_storage_push_array( $var_name, $key, $value ) {
		global $COBBLE_STORAGE;
		if ( ! isset( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $COBBLE_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $COBBLE_STORAGE[ $var_name ][ $key ] ) ) {
				$COBBLE_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $COBBLE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'cobble_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function cobble_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $COBBLE_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $COBBLE_STORAGE[ $var_name ] ) && is_array( $COBBLE_STORAGE[ $var_name ] ) && count( $COBBLE_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $COBBLE_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $COBBLE_STORAGE[ $var_name ][ $key ] ) && is_array( $COBBLE_STORAGE[ $var_name ][ $key ] ) && count( $COBBLE_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $COBBLE_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'cobble_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function cobble_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $COBBLE_STORAGE;
		if ( ! isset( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $COBBLE_STORAGE[ $var_name ][ $key ] ) ) {
			$COBBLE_STORAGE[ $var_name ][ $key ] = 0;
		}
		$COBBLE_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'cobble_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function cobble_storage_concat_array( $var_name, $key, $value ) {
		global $COBBLE_STORAGE;
		if ( ! isset( $COBBLE_STORAGE[ $var_name ] ) ) {
			$COBBLE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $COBBLE_STORAGE[ $var_name ][ $key ] ) ) {
			$COBBLE_STORAGE[ $var_name ][ $key ] = '';
		}
		$COBBLE_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'cobble_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function cobble_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $COBBLE_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $COBBLE_STORAGE[ $var_name ] ) ? $COBBLE_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $COBBLE_STORAGE[ $var_name ] ) ? $COBBLE_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'cobble_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function cobble_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $COBBLE_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $COBBLE_STORAGE[ $var_name ]->$prop ) ? $COBBLE_STORAGE[ $var_name ]->$prop : $default;
	}
}
